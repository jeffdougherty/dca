...
lista = []
def populate_lista_with_classes():
    #Fills lista with all the Class entries in the Ship table of the CAS database
    global lista
    cursor = connect_to_db()
    cmd_string = "SELECT Class,Class_Variant FROM Ship"
    avail_classes = cursor.execute(cmd_string)
    while True:
        next_entry = avail_classes.fetchone()
        if next_entry == None:
            break
        else:
            string_entry = next_entry[0]
            if len(next_entry[1]) > 1:
                add_string = '-' + next_entry[1]
                string_entry += add_string
            #print string_entry
            lista.append(string_entry)
